...
cdef class HistogramBuilder:
    ...
    cdef void _compute_histogram_brute_single_feature_mab(
            HistogramBuilder self,
            const int feature_idx,
            const unsigned int [::1] sample_indices,  # IN
    ...
    def update_histograms(
        ...
        for histogram in new_histograms:
            for bin in histogram:
                if idx == 0:
                    print("bin count:" + str(bin["count"]))
        ...
        with nogil:
            ...
            for f_idx in prange(
                ...
                self._compute_histogram_brute_single_feature_mab(
                    feature_idx, sample_indices, new_histograms
                )
